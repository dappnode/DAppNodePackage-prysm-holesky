...
function ensure_files_exist() {
  ...
  if [ ! -d "${WALLET_DIR}" ]; then
    {
      echo "${ERROR} wallet directory not found, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
  ...
  if [ ! -f "${WALLETPASSWORD_FILE}" ]; then
    {
      echo "${ERROR} wallet password file not found, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
      empty_validator_volume
      exit 1
    }
  fi
...
function ensure_requirements() {
  ...
  if [ "$(curl -s -X GET \
    ...
    --retry-delay 3 \
    --retry-all-errors \
    http://beacon-chain.prysm-holesky.dappnode:3500/eth/v1/beacon/genesis)" == 200 ]; then
    echo "${INFO} web3signer available"
  else
    {
      echo "${ERROR} beacon-chain not available after 3 minutes, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
      empty_validator_volume
      exit 1
    }
  fi
  ...
  if [ "$(curl -s -X GET \
    -H "Content-Type: application/json" \
    -H "Host: prysm.migration-holesky.dappnode" \
    --write-out '%{http_code}' \
    --silent \
    --output /dev/null \
    --retry 30 \
    ...
    {
      echo "${ERROR} web3signer not available after 3 minutes, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
      empty_validator_volume
      exit 1
    }
  fi
...
function get_public_keys() {
  ...
  if VALIDATORS_PUBKEYS="$(validator accounts list \
    ...
    {
      echo "${ERROR} validator accounts list failed, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
      empty_validator_volume
      exit 1
    }
  fi
...
function export_keystores_walletpassowrd() {
  ...
  validator accounts backup \
    ...
    --"${NETWORK}" \
    --accept-terms-of-use || {
    echo "${ERROR} failed to export keystores, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
    empty_validator_volume
    exit 1
  }
  ...
  cp "${WALLETPASSWORD_FILE}" "${BACKUP_WALLETPASSWORD_FILE}" || {
    echo "${ERROR} failed to export walletpassword.txt, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
    empty_validator_volume
    exit 1
  }
  ...
  unzip -d "${BACKUP_KEYSTORES_DIR}" "${BACKUP_ZIP_FILE}" || {
    echo "${ERROR} failed to unzip keystores, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
    empty_validator_volume
    exit 1
  }
}
...
function export_slashing_protection() {
  validator slashing-protection-history export \
    ...
    --accept-terms-of-use || {
    echo "${ERROR} failed to export slashing protection, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
    empty_validator_volume
    exit 1
  }
...
function empty_validator_volume() {
  ...
  mv "/root/.eth2/tosaccepted" "${WALLET_DIR}"/tosaccepted || echo "${WARN} failed to move tosaccepted file, manual migration required. Please check https://holesky-manual-migration.dappnode.io to get more info"
